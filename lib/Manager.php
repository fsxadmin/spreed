...
class Manager {
	...
	public function createParticipantObject(Room $room, array $row) {
		...
		return new Participant($this->db, $room, $row['user_id'], (int) $row['participant_type'], (int) $row['last_ping'], $row['session_id'], (bool) $row['in_call'], (bool) $row['favorite'], (int) $row['last_read_message'], $lastMention);
	}
